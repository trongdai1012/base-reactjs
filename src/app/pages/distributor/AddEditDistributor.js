...
const AddEditDistributor = (props) => {
    // Example 1
    ...
    const permissions = {
        addEditDistributor: 'add-distributor'
    }

    useEffect(() => {
        let check = checkPermission(permissions.addEditDistributor);
        if (check == 2) {
            setRefuse(true);
        } else if (check == 0) {
            setFirstLoad(true);
    ...
    const submitDistributor = () => {
        ...
        enableLoadSubmit();
        makeRequest('post', `distributor/add`, { ...dataAdd, products })
            .then(({ data }) => {
                if (data.signal) {
                    showSuccessMessageIcon('Tạo nhà phân phối thành công!')
                    props.history.push('/distributor/list')
                } else {
                                ...
                                {step === 0 && (
                                    <Form onSubmit={handleSubmit}>
                                        ...
                                        <div className="kt-login__actions">
                                            <Link to="/distributor/list" style={{ marginRight: '5px' }}>
