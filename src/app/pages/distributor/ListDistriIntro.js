...
const ListDistriIntro = (props) => {
    ...
    const permissions = {
        getListDistriIntro: 'get-intro-distri'
    }

    useEffect(() => {
        let check = checkPermission(permissions.getListDistriIntro);
        if (check == 1) {
    ...
    const searchDistributor = (dataSearch = {}) => {
        setLoading(true);
        enableLoadSearch();
        makeRequest('get', `distributor/getAllIntro`, dataSearch)
            .then(({ data }) => {
                if (data.signal) {
    ...
    const renderAction = (row) => {
        return (
            <>
                ...
                {(row.order_condition.status == 0 && user && ((user.type == 1 && !row.distributor_id) || user.distributor_id == row.distributor_id)) && (
                    <>
                        <div className="mg-b5">
                            <Button type="primary" className="button-center-item" size="small" onClick={() => showConfirmPayment(row.order_condition, row.presentee)} icon={<CheckOutlined />}>
                                Xác nhận thanh toán
                            </Button>
                        </div>
                        <div>
                            <Button type="danger" className="button-center-item" size="small" onClick={() => showCancelOrder(row.order_condition, row.presentee)} icon={<DeleteOutlined />}>
                                Huỷ bỏ
                            </Button>
                        </div>
                    </>
    ...
    const showViewOrder = (intro) => {
        setIntroView(intro);
        if (intro.order_condition.status == 1) {
            getOrderKey(intro.order_condition.code);
        } else {
            setOrderKey([]);
        }
    }
    ...
    const showConfirmPayment = (order, distributeBuy) => {
        order.distributor_buy_email = distributeBuy.email
        setDataConfirm({
            show: true,
            order,
    ...
    const showCancelOrder = (order, distributeBuy) => {
        order.distributor_buy_email = distributeBuy.email
        setDataCancel({
            show: true,
            order,
            ...
            <div className="row">
                <div className="col-md-12">
                    <div className="kt-section">
                                        ...
                                        <TableBody>
                                            {rows && rows.length ? rows.map((row, key) => (
                                                <TableRow key={`list-distri-${row.id}`}>
                                                    <TableCell>{renderInfoDistributor(row.presentee)}</TableCell>
                                                    <TableCell className="textindiv">{renderLevel(row.presentee.level)}</TableCell>
                                                    {user && user.distributor_id === 0 &&
                                                        <TableCell>{renderInfoDistributor(row.presenter)}</TableCell>
                                                    }

                                                    <TableCell>{formatMoney(row.order_condition.total_price)} VNĐ</TableCell>

                                                    <TableCell>{formatMoney(row.order_condition.total_price * row.commission_percent / 100)} VNĐ</TableCell>

                                                    <TableCell className="textindiv">{renderStatusText(row.order_condition)}</TableCell>
                                                    <TableCell>
                                                        {renderAction(row)}
                                                    </TableCell>
                                                </TableRow>
                                    ...
                                    <table className="table table-striped table-bordered">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>{renderInfoDistributor(introView.presentee)}</td>
                                            </tr>
                                            <tr>
                                                <td className="fontBold">Giới thiệu bởi</td>
                                                <td>{renderInfoDistributor(introView.presenter)}</td>
                                            </tr>
                                            <tr>
                                                <td className="fontBold">Mã đơn hàng</td>
                                                <td>{introView.order_condition.code}</td>
                                            </tr>

                                            <tr>
                                                <td className="fontBold">Gói</td>
                                                <td>
                                                    {renderPackage(introView.order_condition.productData)}
                                                </td>
                                            </tr>

                                            <tr>
                                                <td className="fontBold">Tổng tiền đơn hàng</td>
                                                <td>{formatMoney(introView.order_condition.origin_price)}</td>
                                            </tr>

                                            <tr>
                                                <td className="fontBold">Chiết khấu</td>
                                                <td>{introView.order_condition.discount}%</td>
                                            </tr>

                                            <tr>
                                                <td className="fontBold">Tổng tiền thực trả</td>
                                                <td>{formatMoney(introView.order_condition.total_price)}</td>
                                            </tr>

                                            <tr>
                                                <td className="fontBold">Hoa hồng giới thiệu</td>
                                                <td>{formatMoney((introView.order_condition.total_price * introView.commission_percent) / 100)} VNĐ</td>
                                            </tr>

                                            <tr>
                                                <td className="fontBold">Thời gian</td>
                                                <td>{formatTime(introView.order_condition.createdAt)}</td>
                                            </tr>
                                            <tr>
                                                <td className="fontBold">Trạng thái</td>
                                                <td>
                                                    {renderStatusText(introView.order_condition)}
                                                </td>
                                            </tr>

                                            {(introView.order_condition.status == 1 || introView.order_condition.status == 2) ? (
                                                <tr>
                                                    <td className="fontBold">{introView.order_condition.status == 1 ? 'Hình thức thanh toán' : 'Lý do huỷ'}</td>
                                                    <td>{introView.order_condition.note_pay}</td>
                                                </tr>
                                            ) : ''}

                                            {(user && !user.distributor_id && introView.order_condition.status == 1) ? (
                                                <tr>
                        ...
                        <Modal
                            title={`Chi tiết key đơn hàng ${introView && introView.order_condition ? introView.order_condition.code : ''}`}
...
export default connect(mapStateToProps, null)(ListDistriIntro);