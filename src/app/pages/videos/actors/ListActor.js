...
export default function ListActor() {
  ...
  const clickModalOk = () => {
    ...
    makeRequest('post', `api/admin/deleteActor?token=${token}`,{idDel})
      .then(({ data }) => {
        if (data.signal) {          
  ...
  return (
    <>
      ...
      {checkPermission(permissions.create) && <Link to="/videos/actors/add" className="btn btn-label-warning btn-bold btn-sm btn-icon-h kt-margin-l-10">Add New</Link>}
      <div className="row">
                    ...
                    <TableBody>
                      { rows.length ?  rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,key) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            {checkPermission(permissions.getOne) &&<Link to={`/videos/actors/edit/${row.id}`} data-toggle="tooltip" data-placement="top" title="Edit actor"><Icon className="fa fa-pen" style={{ color: '#ffa800',fontSize: 15 }} /></Link>}
