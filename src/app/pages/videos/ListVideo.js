...
export default function ListVideo() {
  ...
  const clickModalOk = () => {
    ...
    makeRequest('post', `api/admin/deleteVideo?token=${token}`,{idDel})
      .then(({ data }) => {
        if (data.signal) {          
  ...
  return (
    <>
      ...
      {checkPermission(permissions.create) && <Link to="/videos/add" className="btn btn-label-warning btn-bold btn-sm btn-icon-h kt-margin-l-10">Add New</Link>}
                  ...
                  <Table className={classes1.table}>
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell style={{width:250}}>Updated</TableCell>
                    ...
                    {rows.length ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,key) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            {checkPermission(permissions.getOne) && <Link to={`/videos/edit/${row.id}`} data-toggle="tooltip" data-placement="top" title="Edit video"><Icon className="fa fa-pen" style={{ color: '#ffa800',fontSize: 15 }} /></Link>}
                            {checkPermission(permissions.delete) && <span style={{cursor:'pointer'}} data-toggle="tooltip" data-placement="top" title="Delete video"><Icon className="fa fa-trash" onClick={(e) => showModal(row.id)} style={{ color: 'rgb(220, 0, 78)',fontSize: 15,marginLeft:15 }} /></span>}
                          </TableCell>
                        </TableRow>
                      )):(
                        <TableRow>
