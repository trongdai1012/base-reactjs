...
export default function ListCategory() {
  ...
  useEffect(() => {
    ...
    makeRequest('get', `api/admin/allCategory?token=${token}`)
        .then(({ data }) => {
            if (data.signal) {
  ...
  const clickModalOk = () => {
    ...
    makeRequest('post', `api/admin/deleteCategory?token=${token}`,{idDel})
      .then(({ data }) => {
        if (data.signal) {          
  ...
  const handleSubmit = (e) => {
    e.preventDefault();    
    makeRequest('get', `api/admin/allCategory?token=${token}`,dataSearch)
    .then(({ data }) => {
        if (data.signal) {        
          setPage(0); 
          setRow(data.data)
  ...
  return (
    <>
      ...
      {checkPermission(permissions.create) &&<Link to="/videos/categories/add" className="btn btn-label-warning btn-bold btn-sm btn-icon-h kt-margin-l-10">Add New</Link>}
      <div className="row">
                  ...
                  <Table className={classes1.table}>
                    ...
                    { rows.length ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,key) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                              {checkPermission(permissions.getOne) && <Link to={`/videos/categories/edit/${row.id}`} data-toggle="tooltip" data-placement="top" title="Edit category"><Icon className="fa fa-pen" style={{ color: '#ffa800',fontSize: 15 }} /></Link>}
                              {checkPermission(permissions.delete) && <span style={{cursor:'pointer'}} data-toggle="tooltip" data-placement="top" title="Delete category"><Icon className="fa fa-trash" onClick={(e) => showModal(row.id)} style={{ color: 'rgb(220, 0, 78)',fontSize: 15,marginLeft:15 }} /></span>}
                          </TableCell>
                        </TableRow>
                      )):(
                        <TableRow>
