...
export default function UserHistories(props) {
  // Example 1
  ...
  useEffect(() => {
    ...
    if(id){
      makeRequest('get', `api/admin/getHistoriesWatchVideo?token=${token}`,data)
        .then(({ data }) => {
  ...
  const unfilteredData = (e) =>{
    ...
    makeRequest('get', '/api/admin/getHistoriesWatchVideo',data)
    .then(({ data }) => {
        if (data.signal) {          
            setRow(data.data)
        }
                    ...
                    <TableBody>
                      {rows.length ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,key) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell><img src={row.video.thumbnail} width="100px"/></TableCell>
                          <TableCell>{row.video.title}</TableCell>
