...
export default function LinkCollaborator() {
    ...
    const permissions = {
        getLinkCollab: 'collab-permission'
    }

    useEffect(() => {
        let check = checkPermission(permissions.getLinkCollab);
        if (check == 1) {
            getLink({});
        } else if (check == 2) {
            setRefuse(true);
    ...
    const getLink = () => {
        setLoading(true);
        makeRequest('get', `collaborators/getlinkpaymentcollaborator`)
            .then(({ data }) => {
                if (data.signal) {
                    const res = data.data;
                    setDataLink(res);
                                                    ...
                                                    <div className="form-group" >
                                                        ...
                                                        <div style={{ padding: '10px' }} className="input-group">
                                                            <input readOnly type="text" className="url-free form-control"
                                                                defaultValue={`${URL_PAYMENT}${item.linkPay}`} />
                                                            <CopyToClipboard style={{ marginBottom: '10px' }} text={`${URL_PAYMENT}${item.linkPay}`}
                                                                onCopy={() => {
