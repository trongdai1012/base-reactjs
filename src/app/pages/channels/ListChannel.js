...
export default function ListChannel() {
  ...
  useEffect(() => {
    ...
    makeRequest('get', `api/admin/allChannel?token=${token}`,dataSearch)
        .then(({ data }) => {
            if (data.signal) {
                setRow(data.data)
            }
  ...
  const clickModalOk = () => {
    ...
    makeRequest('post', `api/admin/deleteChannel`,{idDel})
      .then(({ data }) => {
        if (data.signal) {          
  ...
  const unfilteredData = (e) =>{
    ...
    makeRequest('get', `api/admin/allChannel?token=${token}`,{
      keyword: '',
      status: 'all',
      user:userLogin
    })
  ...
  const renderOwner = (row) => {
    if(row.user_id){
      if (row.user.email || row.user.mobile) return row.user.email || row.user.mobile;
      if (row.user.sns[0]) {
          return `${row.user.sns[0].sns_type}_${row.user.sns[0].connect_id}`
      }
    }
    return '-';
}
  return (
    <>
      ...
      {checkPermission(permissions.create) &&<Link to="/channels/add" className="btn btn-label-warning btn-bold btn-sm btn-icon-h kt-margin-l-10">Add New</Link>}
      <div className="row">
                  ...
                  <Table className={classes1.table}>
                    ...
                    {rows.length ? rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row,key) => (
                        <TableRow key={row.id}>
                            ...
                            {/* <TableCell>{renderOwner(row)}</TableCell> */}
                            <TableCell>{renderStatusText(row)}</TableCell>
                          <TableCell>
                              {checkPermission(permissions.getOne) && <Link to={`/channels/edit/${row.id}`} data-toggle="tooltip" data-placement="top" title="Edit channel"><Icon className="fa fa-pen" style={{ color: '#ffa800',fontSize: 15 }} /></Link>}
                              {checkPermission(permissions.delete) && <span style={{cursor:'pointer'}} data-toggle="tooltip" data-placement="top" title="Delete channel"><Icon className="fa fa-trash" onClick={(e) => showModal(row.id)} style={{ color: 'rgb(220, 0, 78)',fontSize: 15,marginLeft:15 }} /></span>}
                          </TableCell>
                        </TableRow>
                      )):(
                        <TableRow>
