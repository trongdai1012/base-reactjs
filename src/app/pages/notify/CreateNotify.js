...
import { DISTRIBUTOR_LEVEL } from '../../config/notify';
...
const CreateNotify = (props) => {
    ...
    const permissions = {
        createNotify: 'add-notify'
    }

    useEffect(() => {
        let check = checkPermission(permissions.createNotify);
        if (check == 2) {
            setRefuse(true);
        } else if (check == 0) {
            setFirstLoad(true);
    ...
    const handleSubmit = (e) => {
        ...
        if (!dataAdd.level) {
            return showErrorMessage('Vui lòng chọn cấp nhà phân phối nhận thông báo');
        }
        ...
        setLoading(true);
        makeRequest('post', `notification/createnotification`, dataAdd)
            .then(({ data }) => {
                if (data.signal) {
                    resetForm();
...
export default connect(mapStateToProps, null)(CreateNotify);