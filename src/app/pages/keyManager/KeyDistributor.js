...
export default function KeyDistributor() {
    ...
    const searchKey = () => {
        setLoading(true);
        makeRequest('get', `distributor/getKey`, {})
            .then(({ data }) => {
                if (data.signal) {
                    const res = data.data;
                    setDataKey(res);
    ...
    const renderPackage = () => {
        return (packageAll && packageAll.length > 0)
            ? packageAll.map(item => {
                return (
                    <option value={item.id} key={`package-distri-${item.id}`} >{item.name}</option>
                )
            })
            : null
    }
    ...
    const Export = () => {
        makeRequest('get', `productkey/export`, dataSearch)
            .then(({ data }) => {
                if (data.signal) {
                    hideModalImport();
                    unfilteredData();
    ...
    return (
        <>
            ...
            <Link onClick={() => Export()} className="btn btn-primary btn-bold btn-sm btn-icon-h kt-margin-l-10">Export key</Link>
            <div className="row">
