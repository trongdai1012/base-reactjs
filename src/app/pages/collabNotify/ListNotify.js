...
import DatePicker from '../../common/antd/component/DatePicker';
...
const ListNotityCollab = (props) => {
    ...
    const deleteNotify = () => {
        if (deleteID != 0) {
            setLoadDelete(true);
            makeRequest('get', `collabnotify/delete?id=${deleteID}`).then(({ data }) => {
                if (data.signal) {
    ...
    const getNotifyUpdate = (id) => {
        makeRequest('get', `collabnotify/getById?id=${id}`).then(({ data }) => {
            if (data.signal) {
    ...
    const handleSubmitUpdate = (e) => {
        ...
        setLoadUpdate(true);
        makeRequest('post', `collabnotify/update`, dataUpdate)
            .then(({ data }) => {
                if (data.signal) {
    ...
    return (
        <>
            <Link to="/notifycollab/create" className="btn btn-primary btn-bold btn-sm btn-icon-h kt-margin-l-10">Tạo thông báo</Link>
...
export default connect(mapStateToProps, null)(ListNotityCollab);